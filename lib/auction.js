...
export function pickRandomPlayer(players, conditions) {
  const lastIds = conditions.lastPlayerIds || [];
  ...
  const normalPool = players.filter((p) => baseFilter(p) && !lastIds.includes(p.id));
  ...
  const lastPool = players.filter((p) => baseFilter(p) && lastIds.includes(p.id));
